...
def lua_table_to_list(x, length: Optional[int] = None, low_index: int = 1):
    if not x:
...
class ResultProc:
    def __init__(self, result, enc):
        ...
        self._enc = enc
        self._i = 1
        self._vlen = len(result) if result else 0
    def forward(self):
        self._i += 1
    def back(self):
        self._i -= 1
    ...
    def take_dict(self, decode_bytes=True):
        ...
        return x
    def take_list(self, length: Optional[int] = None):
    ...
    def take_option_string_bool(self):
        ...
        if p is None or p is True or p is False:
            ...
            return p
        return self.take_string()
    def take_list_of_strings(self, length: Optional[int] = None):
    ...
    def take_decoded(self):
        """Take 一个值，并按 self._enc 递归解码其中的 bytes。"""
        return _decode_rec(self._enc, self.take())
    def isend(self):
        return self._i > self._vlen