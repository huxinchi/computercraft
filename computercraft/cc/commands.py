...
__all__ = (
    'exec', 'execAsync', 'list',
    'getDimension', 'getBlockPosition',
    'getBlockInfo', 'getBlockInfos',
    'getEntities',
)

def execAsync(command: str) -> int:
    """Asynchronously execute a command.
    Returns the task id. New in version 1.7."""
    return eval_lua(b'G:commands:M:execAsync', command).take_int()


def getDimension() -> str:
    """Get the name of the dimension.
    New in version 1.7."""
    return eval_lua(b'G:commands:M:getDimension').take_string()


from typing import Any, List
def getEntities(selector: str) -> List[Any]:
    return eval_lua(b'G:commands:M:getEntities', selector).take_list()
